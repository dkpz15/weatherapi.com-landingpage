...
function ScrollSpy() {
  ...
  var callback_OnChange =
      typeof settings["callbackOnChange"] !== "undefined"
        ? settings["callbackOnChange"]
        : function () {},
    callback_OnDestroy =
      typeof settings["callbackOnDestroy"] !== "undefined"
        ? settings["callbackOnDestroy"]
        : function () {},
  ...
  var self = {
    ...
    Indicator: function Indicator() {
      ...
      if (
        settings_indicator["element"] !== null ||
        typeof settings_indicator["element"] !== "undefined"
      ) {
        ...
        if (
          typeof settings_indicator["indicator_container_class"] !== "undefined"
        ) {
          indicator.classList.add(
            settings_indicator["indicator_container_class"]
          );
        }
        ...
        Array.prototype.forEach.call(SpySections, function (element) {
          ...
          if (
            typeof settings_indicator["indicator_item_class"] !== "undefined"
          ) {
            indicator_item.classList.add(
              settings_indicator["indicator_item_class"]
            );
          }
        ...
        hasIndicator = true;
        const spyClickable = Array.from(
          document.querySelectorAll(".spy-clickable")
        );

        spyClickable.map((spyli) => {
          if (
            spyli.innerHTML == "Introduction" ||
            spyli.innerHTML == "APIs" ||
            spyli.innerHTML == "Resources"
          ) {
            spyli.classList.add("add-font-size");
          }
          ...
          if (
            ...
            spyli.classList.add("add-margin-left");
          }

          if (
            spyli.innerHTML == "Getting Started" ||
            spyli.innerHTML == "Realtime API" ||
            spyli.innerHTML == "Weather Icons and Codes"
          ) {
            spyli.classList.add("add-margin-top");
          }

          if (
            spyli.innerHTML == "API Error Codes" ||
            spyli.innerHTML == "Integrations" ||
            spyli.innerHTML == "Credit"
          ) {
            spyli.classList.add("add-margin-bottom");
          }

          if (spyli.innerHTML == "Weather Alerts") {
            spyli.innerHTML = `Weather Alerts <span class="badge rounded-pill bg-danger badge-new ms-3"
            >NEW</span
          >`;
          } else if (spyli.innerHTML == "Air Quality") {
            spyli.innerHTML = `Air Quality <span class="badge rounded-pill bg-danger badge-new ms-3"
            >NEW</span
          >`;
          } else if (spyli.innerHTML == "Bulk Request") {
            spyli.innerHTML = `Bulk Request <span class="badge rounded-pill bg-danger badge-new ms-3"
            >NEW</span
          >`;
          } else if (spyli.innerHTML == "Future API") {
            spyli.innerHTML = `Future API <span class="badge rounded-pill bg-danger badge-new ms-3"
            >NEW</span
          >`;
          }

          if (spyli.innerHTML == "Introduction") {
            spyli.innerHTML = `<div class="border-font-awesome"><i class="fa-solid fa-signs-post"></i></div><a href="#introduction" class="a-font-awesome">Introduction</a>`;
            spyli.classList.add("border-left-none");

            spyli.classList.add("spy-active");
          }
          if (spyli.innerHTML == "APIs") {
            spyli.innerHTML = `<div class="border-font-awesome"><i class="fa-solid fa-box"></i></div><a href="#apis" class="a-font-awesome">APIs</a>`;
            spyli.classList.add("border-left-none");
          }
          if (spyli.innerHTML == "Resources") {
            spyli.innerHTML = `<div class="border-font-awesome"><i class="fa-solid fa-book-open-reader"></i></div><a href="#resources" class="a-font-awesome">Resources</a>`;
            spyli.classList.add("border-left-none");
          }
        });
      }
  ...
  var CheckIsInView = function CheckIsInView(element) {
    var ScrollPos =
      arguments.length > 1 && arguments[1] !== undefined
        ...
        : ScrollPosition();
    return (
      (element.offsetTop > ScrollPos[0] && element.offsetTop < ScrollPos[1]) ||
      (element.offsetTop + element.offsetHeight > ScrollPos[0] &&
        element.offsetTop + element.offsetHeight < ScrollPos[1]) ||
      (element.offsetTop < ScrollPos[0] &&
        element.offsetTop + element.offsetHeight > ScrollPos[1])
    );
  };
  ...
  var GetVisibilityDistanceFromCenter =
    function GetVisibilityDistanceFromCenter(element) {
      var ScrollPos =
        arguments.length > 1 && arguments[1] !== undefined
          ? arguments[1]
          : ScrollPosition();
  ...
  var ScrollToSection = function ScrollToSection(element) {
    ...
    if (element.offsetHeight > page_visible_height) {
      ...
      offset =
        element.offsetTop - (page_visible_height - element.offsetHeight) / 2;
    }
  ...
  var OnScroll = function OnScroll() {
    ...
    var processScroll = function processScroll() {
      ...
      Array.prototype.forEach.call(SpySections, function (element) {
        var Percent = [
          element[0].offsetTop,
          GetVisibilityPercent(element[0], ScrollPos),
        ];
      ...
      if (isNaN(max)) {
        ...
        Array.prototype.forEach.call(
          StagesPositionPercentage,
          function (objectPositionPercentage) {
            Array.prototype.forEach.call(SpySections, function (element) {
              if (element[0].offsetTop === objectPositionPercentage[0]) {
                var DistanceFromCenter = [
                  element[0].offsetTop,
                  GetVisibilityDistanceFromCenter(element[0], ScrollPos),
                ];

                if (
                  !TempStagesPositionPercentage.includes(DistanceFromCenter)
                ) {
      ...
      if (max !== lastItemPercent) {
        ...
        Array.prototype.forEach.call(
          ...
          function (objectPositionPercentage) {
            ...
            if (
              objectPositionPercentage[1] === max &&
              objectPositionPercentage[0] !== CurrentPositionTop &&
              !fakePercent
            ) {
  ...
  var InitializeOnScrollEvent = function InitializeOnScrollEvent() {
    var SpySectionsObject = document.getElementsByClassName(
      settings["contexts_class"]
    );
...
let spyClickable = function () {
  const spyClickable = Array.from(document.querySelectorAll(".spy-clickable"));

  spyClickable.map((spyli) => {
    if (
      spyli.classList.contains("add-font-size") &&
      spyli.classList.contains("spy-active")
    ) {
      spyli.childNodes[1].classList.add("a-color-blue");
      spyli.childNodes[0].classList.add("border-font-awesome-blue");
      spyli.childNodes[0].firstElementChild.classList.add("font-awesome-white");
    } else if (
      spyli.classList.contains("add-font-size") &&
      !spyli.classList.contains("spy-active")
    ) {
      spyli.childNodes[1].classList.remove("a-color-blue");
      spyli.childNodes[0].classList.remove("border-font-awesome-blue");
      spyli.childNodes[0].firstElementChild.classList.remove(
        "font-awesome-white"
      );
    }

    spyli.addEventListener("mouseover", function () {
      if (
        spyli.classList.contains("add-font-size") &&
        !spyli.classList.contains("spy-active")
      ) {
        spyli.childNodes[1].classList.add("hover-for-intro-apis-resources");

        spyli.addEventListener("mouseout", function () {
          spyli.childNodes[1].classList.remove(
            "hover-for-intro-apis-resources"
          );
        });
      }
    });
